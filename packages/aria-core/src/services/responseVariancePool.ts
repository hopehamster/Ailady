 ...
 *  - Picker biases away from the last K used IN THIS PROCESS for the same user.
 *
 * PHASE-0 PORT NOTE (brain decouple): the legacy module write-through-cached
 * recency to a Firestore-backed RecencyTracker (L6) for cross-cold-start
 * anti-repeat. That persistence is Phase-1 memory work — here recency is
 * IN-PROCESS ONLY (per-isolate). `pickVariant` is unchanged (it always read the
 * in-process cache synchronously); `notifyPicked` no longer fires a Firestore
 * save and `primeRecency` is a no-op until the memory layer lands.
 *
...
// PHASE-0: in-process only (no Firestore write-through yet).
const MAX_USERS = 200;
...
/**
 * Internal: stamp a freshly-chosen variant into the warm cache.
 * PHASE-0: in-process cache only — the Firestore write-through is deferred to
 * the Phase-1 memory layer.
 */
...
/**
 * PHASE-0: no-op. The legacy implementation warmed the in-process recency cache
 * from Firestore for cross-cold-start anti-repeat; that persistence is Phase-1
 * memory work. Signature preserved so callers don't change.
 */
