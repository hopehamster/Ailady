...
implements ResponsePatternHistoryTracker {
  ...
  async load(_uid: string): Promise<ResponsePatternHistoryState> {
    // PHASE-0 STUB: persistence is Phase-1 memory work.
    // The Firestore read is no-op'd; return empty state so the pure
    // detection path runs with no history (the brain runs memory:null
    // in Phase 0). The in-process LRU cache above is intentionally NOT
    // consulted here — a Phase-0 load is always cold/empty.
    return { entries: [] };
  }

  async save(uid: string, state: ResponsePatternHistoryState): Promise<void> {
    // PHASE-0 STUB: persistence is Phase-1 memory work.
    // The Firestore write is no-op'd. We still mirror into the in-process
    // LRU cache so a same-process record→read sequence stays coherent
    // within a single invocation, but nothing is durably persisted.
