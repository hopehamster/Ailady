...
function selectRecentExchangeEvidence(
  ...
  for (let index = 0; index < loops.length; index += 1) {
    ...
    evidence.push({
      ...
      observedAtMs: loop.lastMentionedAt ?? nowMs - 60000 - index * 1000,
      createdAtMs: loop.createdAt,
      confidence: Math.max(0.4, Math.min(0.95, loop.priority || 0.6)),
      isResolved: loop.status === 'resolved',
      isExpired: !!loop.expiresAt && loop.expiresAt <= nowMs,
  ...
  const currentEvent =
    cue && currentSummary
      ? ({
          ...
          confidence: cue.confidence,
          status: cue.direction === 'past' ? 'resolved' : 'open',
          createdAt: temporal.now.getTime(),
          lastMentionedAt: temporal.now.getTime(),
        } as ChronologyEvent)
      : null;
