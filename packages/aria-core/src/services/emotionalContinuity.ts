 ...
 *                                                 redeploy.
 *
 * PHASE-0 PORT NOTE: This is the Firebase-free aria-core port. The
 * FirestoreEmotionalContinuityTracker load()/save() bodies are STUBBED to
 * Phase-0 no-ops (load returns empty/default ContinuityState; save is a
 * no-op). Persistence is Phase-1 memory work. The InMemory tracker and ALL
 * pure compute/prompt helpers are preserved verbatim.
 */
...
// ──────────────────────────────────────────────────────────────────────────
// Firestore-backed tracker (PHASE-0: persistence stubbed)
// ──────────────────────────────────────────────────────────────────────────
  ...
  implements EmotionalContinuityTracker
{
  ...
  async load(_uid: string): Promise<ContinuityState> {
    // PHASE-0 STUB: persistence is Phase-1 memory work.
    // Returns the empty/default ContinuityState (no snapshots, never written).
    void snapshotFromRaw; // retained for Phase-1; referenced to avoid dead-code pruning churn
    return { recent: [], lastWrittenTurn: LAST_WRITTEN_TURN_UNSET };
  }

  async save(_uid: string, _snapshot: ToneSnapshot): Promise<void> {
    // PHASE-0 STUB: persistence is Phase-1 memory work. No-op.
    return;
  }
