 ...
 *
 * Node 24 import discipline: imports FieldValue + getFirestore directly from
 * 'firebase-admin/firestore' (NOT the lazy admin.firestore.X namespace, which
 * is undefined on Node 24).
 */
...
const POLICY_TRAJECTORY_DOC = 'policyTrajectory';
...
export class FirestorePolicyTrajectoryTracker implements PolicyTrajectoryTracker {
  ...
  private readonly userWindows = new Map<string, PolicyTrajectoryEntry[]>();
  /** Per-uid in-flight load promise — coalesces concurrent loads so multiple
   *  near-simultaneous turns for the same user don't fire N parallel reads. */
  private readonly inflightLoads = new Map<string, Promise<PolicyTrajectoryEntry[]>>();
   ...
   *  Firestore round-trip before computing its own next state. */
  ...
  async loadWindow(uid: string): Promise<PolicyTrajectoryEntry[]> {
    ...
    if (cached) {
      // Bump LRU on cache hit: re-touch so this uid moves to the
      // most-recently-used end of the insertion-order Map. Without this, an
      // active user whose window is queried repeatedly never gets re-promoted
      // and can be evicted by a burst of new uids while still hot. Use a
      // defensive copy so the snapshot returned to the caller and the
      // cached array remain independent.
    ...
    const loadPromise = (async (): Promise<PolicyTrajectoryEntry[]> => {
      try {
        ...
        const parsed: PolicyTrajectoryEntry[] = [];
        for (const item of rawEntries) {
          if (!item || typeof item !== 'object') continue;
          const e = item as Record<string, unknown>;
          const warmth = typeof e.warmth === 'number' ? e.warmth : NaN;
          const curiosity = typeof e.curiosity === 'number' ? e.curiosity : NaN;
          const depth = typeof e.depth === 'number' ? e.depth : NaN;
          const playfulness = typeof e.playfulness === 'number' ? e.playfulness : NaN;
          const timestamp = typeof e.timestamp === 'number' && Number.isFinite(e.timestamp)
            ? e.timestamp
            : 0;
          if (
            !Number.isFinite(warmth) ||
            !Number.isFinite(curiosity) ||
            !Number.isFinite(depth) ||
            !Number.isFinite(playfulness)
          ) {
            continue;
          }
          parsed.push({
          ...
          if (parsed.length >= MAX_HISTORY) break;
        }
        this.touchUser(uid, parsed);
        return parsed.map((e) => ({ ...e }));
      } catch (err: unknown) {
        const message = err instanceof Error ? err.message : String(err);
        functions.logger.warn('[PolicyTrajectoryTracker] load failed — using empty window', {
          uid,
          error: message,
        });
        return [];
  ...
  async appendDecision(uid: string, plan: PolicyVector): Promise<void> {
    ...
    // read sees the prior append's mutated cache + Firestore state. This
    // closes the lost-update race where two appends could both read the
    // same prior window, each prepend their own entry, and the second write
    // clobber the first. Errors from the previous append are intentionally
    // swallowed here — they were already logged inside that call — so a
    // failed prior write does not poison the next append's queue slot.
    ...
    const runAppend = async (): Promise<void> => {
      ...
      // Always update the warm-path cache synchronously so the NEXT
      // in-process turn for this uid sees the up-to-date trajectory even if
      // the Firestore write hasn't flushed yet. After awaiting `previous`
      // above, the cache reflects the most-recent committed (or queued-then-
      // cached) state, so this read is race-free against same-uid concurrency.
      const cached = this.userWindows.get(uid);
      ...
      try {
        ...
        const message = err instanceof Error ? err.message : String(err);
        functions.logger.warn('[PolicyTrajectoryTracker] save failed — cache only', {
          uid,
          error: message,
        });
      }
