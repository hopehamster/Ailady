 ...
 *
 * Per `oreilly_ai_perf.md` Finding #6 + A4: long prompts hurt TTFT (prefill
 * cost is compute-bound and scales linearly with prompt length); they also
 * trigger the "lost in the middle" performance degradation. Compaction
 * keeps recent recall sharp while bounding cost.
 *
 * Per `round_11.md` Pattern A: the recommended companion-memory shape is
 * working-memory deque (recent N verbatim) + episodic summary (older turns
 * compressed). This module implements the deque + the summarizer.
 *
 ...
 *
 * Phase 3 ships the MODULE + tests. Wire-in into llmService dispatch is
 * deferred to Phase 2 Session-β where the LLM call path is clean.
 */
...
export async function compactHistory(
  ...
  try {
    ...
    functions.logger.warn('historyCompactor: summarizer failed, keeping verbatim', {
