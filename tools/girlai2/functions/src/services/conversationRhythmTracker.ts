 ...
 * Persistence
 * -----------
 * Firestore: `users/{uid}/humanityState/conversationRhythm`
 *   {
...
export class FirestoreRhythmTracker implements RhythmTracker {
    ...
    const loadPromise = (async (): Promise<RhythmWindow> => {
      try {
        ...
        const window: RhythmWindow = {
          userWordCounts: sanitize(data.userWordCounts),
          assistantWordCounts: sanitize(data.assistantWordCounts),
        };
        this.touchUser(uid, {
        ...
        functions.logger.warn('[ConversationRhythmTracker] load failed — using empty window', {
          uid,
          error: message,
        });
        return { userWordCounts: [], assistantWordCounts: [] };
  ...
  async getHint(uid: string): Promise<RhythmHint> {
    ...
    try {
      ...
      const message = err instanceof Error ? err.message : String(err);
      functions.logger.warn('[ConversationRhythmTracker] getHint failed', {
        uid,
        error: message,
      });
      return { preferred: null, confidence: 0, sampleSize: 0, reason: 'insufficient-data' };
  ...
  async observe(observation: RhythmObservation): Promise<void> {
    ...
    try {
      ...
      await docRef.set(
        {
          userWordCounts: window.userWordCounts,
          assistantWordCounts: window.assistantWordCounts,
      ...
      const message = err instanceof Error ? err.message : String(err);
      functions.logger.warn('[ConversationRhythmTracker] observe save failed — cache only', {
        uid: observation.uid,
        error: message,
      });
    }
