// ============================================================================
// db.ts — typed D1 helpers. Thin layer over env.DB.
// ============================================================================
import type { JwkEC } from "./jwt";
...
export async function findUserByPhoneHash(db: D1Database, phoneHash: string): Promise<UserRow | null> {
  return db
    .prepare("SELECT * FROM users WHERE phone_hash = ? LIMIT 1")
    .bind(phoneHash)
    .first<UserRow>();
}
...
export async function createUser(
  ...
  await db
    .prepare(
      `INSERT INTO users (uid, phone_e164, phone_hash, created_at, status, migrated_from, legacy_uid)
       VALUES (?, ?, ?, ?, 'active', ?, ?)`
    )
...
export async function touchLastLogin(db: D1Database, uid: string): Promise<void> {
  const now = Math.floor(Date.now() / 1000);
  await db.prepare("UPDATE users SET last_login_at = ? WHERE uid = ?").bind(now, uid).run();
}
...
export async function incOtpAttempts(db: D1Database, sessionId: string): Promise<void> {
  await db
    .prepare("UPDATE otp_sessions SET attempts = attempts + 1 WHERE session_id = ?")
    .bind(sessionId)
    .run();
}
