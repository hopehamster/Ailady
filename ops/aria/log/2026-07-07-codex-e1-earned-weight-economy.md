...
- **Live baseline:** Ran `pnpm aria:talk` against current staging after the code commit. Staging is still the pre-E1/flag-OFF deploy, and #43 reproduced: Aria over-endorsed quitting the job for a food-delivery startup, backpedaled when asked for honesty, and leaked a truncated callback line. This is the comparison point for the staging flag-on smoke.
...
- **Open items:** Run staging/local flag-on `pnpm aria:talk` smoke and inspect whether honesty under praise/challenge improves. Keep `PSYCHE_EARNED_WEIGHT_ENABLED=false` in prod until evidence is good. Codex could not deploy staging in this shell because `wrangler whoami` reports not authenticated. B1 inner-state flag smoke remains separate; B2 consensus and H3 identity-core remain owner-gated.
