...
// explicit real vendor (OTP_VENDOR === 'plivo'). Any other value — unset, empty,
// a typo, or literally 'mock' — THROWS in prod so a misconfigured deploy 503s
// instead of silently accepting every OTP via the Mock (which always "succeeds"
// on send and verifies any code it minted). The Mock may ONLY be selected when
// ENV === 'dev'. The caller turns the throw into a 503 vendor_unavailable.
// ----------------------------------------------------------------------------
