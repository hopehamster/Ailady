...
- Testing protocol update from owner: when testing Aria as a deployed experience, use Claude's robust Playwright resources in the global `.claude` folder (`C:\Users\Owner\.claude\skills\playwright-best-practices\SKILL.md` and `C:\Users\Owner\.claude\rules\playwright-as-browser-control*`) alongside repo Playwright tests. `aria:talk` is the fast psyche loop; Playwright/browser checks are required for actual app experience, auth, voice/avatar/UI, screenshots, console/network errors, and regression evidence.
